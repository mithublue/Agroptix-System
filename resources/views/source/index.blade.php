            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                        ...
                        <form method="GET" action="{{ route('sources.index') }}" class="grid grid-cols-1 md:grid-cols-{{ auth()->user()->can('manage_source') ? '6' : '5' }} gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="q" name="q"
                                       ...
                                       placeholder="Search type, production method, address, state, country, owner, or exact id/area">
                            </div>
                            <!-- Type Filter -->
                            <div>
                    ...
                    <div class="overflow-x-auto">
                        @can('delete_source')
                        <form id="bulk-delete-sources" method="POST" action="{{ route('sources.bulk-destroy') }}">
                            @csrf
                            @method('DELETE')
                            ...
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        <input type="checkbox" id="select-all-sources" class="rounded border-gray-300">
                                    </th>
                                    @endcan
                                ...
                                @forelse($sources as $source)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <input type="checkbox" name="ids[]" value="{{ $source->id }}" class="row-checkbox-source rounded border-gray-300">
                                        </td>
                                        @endcan
                                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <div class="px-6 py-4 flex items-center justify-between">
                            <div>
                                <span id="selected-count-sources" class="text-sm text-gray-600">0 selected</span>
                            </div>
                            <div>
                                <button type="submit" form="bulk-delete-sources" id="bulk-delete-btn-sources" disabled class="px-4 py-2 bg-red-600 text-white text-sm rounded disabled:opacity-50">Delete Selected</button>
                            </div>
                        </div>
                        </form>
                        @endcan
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const checkboxes = document.querySelectorAll('.row-checkbox-source');
    const deleteBtn = document.getElementById('bulk-delete-btn-sources');
    const selectedCount = document.getElementById('selected-count-sources');
