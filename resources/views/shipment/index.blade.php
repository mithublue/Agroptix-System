<x-app-layout>
    ...
    <div class="py-12" x-data="shipmentIndex()">
        ...
        <x-shipment.shipment-form-drawer>
            <x-shipment.shipment-form :batches="$batches" />
        </x-shipment.shipment-form-drawer>

        <!-- Shipment Show Drawer -->
        <div x-show="showViewDrawer" class="fixed inset-0 overflow-hidden z-50"
            x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200">
            <div class="absolute inset-0 bg-gray-500 bg-opacity-75 transition-opacity" @click="showViewDrawer = false"
                aria-hidden="true"></div>
            <div class="fixed inset-y-0 right-0 pl-10 max-w-full flex" x-on:click.away.stop
                x-on:keydown.escape.window="showViewDrawer = false" x-show="showViewDrawer"
            ...
            <div class="mb-6 bg-white p-4 rounded-lg shadow">
                ...
                <form method="GET" action="{{ route('shipments.index') }}" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                        ...
                        <!-- Vehicle Type Filter -->
                        <div>
                            <label for="vehicle_type" class="block text-sm font-medium text-gray-700 mb-1">Vehicle
                                Type</label>
                        ...
                        <div>
                            ...
                            <select id="mode" name="mode"
                                ...
                                @foreach(['Road', 'Sea', 'Rail', 'Air', 'Multimodal'] as $mode)
                                    <option value="{{ $mode }}" {{ request('mode') == $mode ? 'selected' : '' }}>{{ $mode }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="flex space-x-2">
                            <button type="submit"
                                class="h-10 inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                Apply Filters
                            </button>
                            <a href="{{ route('shipments.index') }}"
                                class="h-10 inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                Reset
                            </a>
                        </div>
                    </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                                ...
                                @foreach($shipments as $shipment)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if($shipment->batch)
                                                <a href="{{ route('batches.show', $shipment->batch) }}"
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex justify-end space-x-2">
                                                @can('view_shipment')
                                                    <button @click="viewShipment({{ $shipment->id }})"
                                                        class="text-indigo-600 hover:text-indigo-900">
                ...
                @else
                    <div class="p-6 text-center text-gray-500">
                        No shipments found.
                        @can('create_shipment')
                            <a href="javascript:void(0)" @click="$dispatch('shipment-form-drawer:show')"
