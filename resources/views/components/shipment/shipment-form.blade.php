<form id="shipment-form" method="POST" action="{{ isset($shipment) ? route('shipments.update', $shipment) : route('shipments.store') }}">
    ...
    <div class="space-y-6">
        ...
        <!-- Route Distance and CO2 Estimate -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
            ...
            <div>
                <label for="co2_estimate" class="block text-sm font-medium text-gray-700">CO₂ Estimate (kg)</label>
                <input type="number" step="0.01" id="co2_estimate" name="co2_estimate" value="{{ old('co2_estimate', $shipment->co2_estimate ?? '') }}"
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                @error('co2_estimate')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
