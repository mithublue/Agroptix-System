<div x-data="labTestingForm()" x-init="init()">
    <form method="POST" :action="formAction" x-on:submit.prevent="submitForm()">
        @csrf
                    ...
                    fetch(form.action, {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                {{--window.location.href = data.redirect || '{{ route("quality-tests.index", batch.id) }}';--}}
                            } else {
