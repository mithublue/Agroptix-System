        ...
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-lg p-8">
                ...
                <form method="POST" action="{{ route('admin.options.saveGeneralSettings') }}"
                    enctype="multipart/form-data" x-data="{ previewUrl: null }">
                    @csrf

                    <!-- Project Name -->
                    <div class="mb-6">
                        <label for="project_name" class="block font-medium text-gray-700 text-lg mb-2">Project
                            Name</label>
                        <input type="text" name="project_name" id="project_name"
                            value="{{ old('project_name', option('project_name', config('app.name', ''))) }}"
                        ...
                        @enderror
                        <p class="text-gray-500 text-sm mt-1">This name will be displayed throughout the application.
                        </p>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        @if(option('favicon'))
                            <div class="mb-3 p-3 bg-gray-50 rounded-md inline-block">
                                ...
                                <img src="{{ asset('storage/' . option('favicon')) }}" alt="Current Favicon"
                        ...
                        @error('favicon')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                        <p class="text-gray-500 text-sm mt-1">Upload a favicon image (ICO, PNG, JPG, SVG). Max size:
                            2MB. Recommended: 32x32px or 16x16px.</p>
